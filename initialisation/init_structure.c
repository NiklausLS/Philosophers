...
/*   Updated: 2024/10/09 22:12:55 by nileempo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_philo_array(t_data *data)
{
	...
	while (i < data->philo_nbr)
	{
		...
		data->philo_array[i].right_fork
			= &data->fork_array[(i + 1) % data->philo_nbr];
...
void	make_philo_threads(t_data *data)
{
	...
	while (i < data->philo_nbr)
	{
		if (pthread_create(&data->philo_array[i].thread,
				NULL, philo_routine, &data->philo_array[i]) != 0)
